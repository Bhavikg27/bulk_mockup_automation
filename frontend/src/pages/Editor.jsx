...
import { Upload, Save, Play, Layers, Check, RefreshCw, Columns } from 'lucide-react';
import { uploadMockup, saveConfig, generateMockup, generateBulkMockups, getMockups, getImageUrl, getPreview } from '../services/api';
...
const Editor = () => {
    ...
    useEffect(() => {
        const updateLayout = () => {
            if (containerRef.current) {
                ...
                setLayout({ scale: newScale, offsetX: newOffsetX, offsetY: newOffsetY });
    ...
    const handleDragMove = (index, e) => {
        ...
        // Use layout state for calculation
    ...
    return (
        <div className="h-full flex flex-col gap-6">
             ...
             <div className="flex justify-between items-center bg-white/80 backdrop-blur-md p-3 rounded-lg shadow-sm border border-gray-200">
                <h2 className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">Editor Space</h2>
                <div className="flex gap-2">
                    <button 
                        onClick={() => setSideBySide(!sideBySide)}
                        className={clsx("flex items-center px-3 py-1.5 rounded-md text-sm font-medium transition-colors", sideBySide ? "bg-primary text-white shadow-lg shadow-primary/30" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}
                    >
            ...
            <div className="flex flex-col lg:flex-row gap-6 flex-1 h-0">
                            ...
                            <>
                                    ...
                                    <Layer>
                                        ...
                                        <Line
                                            points={[
                                                ...
                                                return v * layout.scale + (i % 2 === 0 ? layout.offsetX : layout.offsetY); 
                                            })} 
                                            stroke="#3b82f6"
                                        ...
                                        {points.map((pt, i) => (
                                            <Circle
                                                ...
                                                radius={8 / zoom} // Keep handles constant size visual
                                                fill="#3b82f6"
                                                stroke="white"
                                                strokeWidth={2 / zoom}
                                                draggable
                                ...
                                <div className="absolute bottom-4 left-4 flex space-x-2">
                                     <div className="bg-white/90 backdrop-blur shadow-lg rounded-lg border border-gray-200 p-1 flex items-center space-x-1">
                                        <button onClick={() => handleZoom(-0.1)} className="p-1.5 hover:bg-gray-100 rounded text-gray-600">-</button>
                ...
                {sideBySide ? (
                     <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col relative">
                        <div className="bg-gray-50 p-3 border-b border-gray-100 font-medium text-gray-700">Live Result</div>
                        <div className="flex-1 bg-gray-100 flex items-center justify-center p-4">
                            {previewUrl ? (
                                <div className="relative shadow-lg rounded-lg overflow-hidden group">
                                    <img src={imageUrl} alt="base" className="max-h-[500px] object-contain" />
                                    <img src={previewUrl} alt="warp" className="absolute inset-0 w-full h-full object-contain mix-blend-multiply" /> 
                                </div>
                            ) : (
                                <p className="text-gray-400 text-sm">Upload a design and set points to see result</p>
                            )}
                        </div>
                     </div>
                ) : (
                    <div className="w-full lg:w-80 flex flex-col space-y-6 overflow-y-auto">
                        ...
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                            ...
                            {!mockupId ? (
                                <label className="block w-full border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:bg-gray-50 cursor-pointer transition-colors">
                                    ...
                                    <span className="text-sm font-medium text-gray-600">Upload Base Image</span>
                                    <input type="file" className="hidden" onChange={handleFileUpload} accept="image/*" />
                                </label>
                            ) : (
                                <div className="space-y-4">
                                    <div className="text-sm text-gray-600 truncate">
                                        Using: <span className="font-medium text-gray-900">{mockupName}</span>
                                    </div>
                                    <button 
                        ...
                        {/* Generate Card */}
                        <div className={clsx("bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-opacity", !mockupId && "opacity-50 pointer-events-none")}>
                            <h3 className="font-bold text-gray-900 flex items-center mb-4">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-2">Upload Designs ({designs.length})</label>
                                    <div className="flex gap-2">
                                         <input 
                                            type="file" 
                                            multiple
                                    ...
                                    {designs.length > 0 && (
                                        ...
                                        >
                                            <RefreshCw className="w-3 h-3 mr-1" /> Refresh Preview (First Design)
                                        </button>
                                    )}
                                </div>
                                ...
                                >
                                    {generating ? 'Processing...' : `Generate ${designs.length > 1 ? 'All ' + designs.length : 'Mockup'}`}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                            <h4 className="font-bold text-blue-900 text-sm mb-2">How to use</h4>
                            <ul className="text-xs text-blue-800 space-y-1.5 list-disc pl-4">
                                ...
                                <li>Upload a design to see a <strong>Live Preview</strong>.</li>
